...
self.addEventListener('fetch', function (event) {
  ...
  event.respondWith(
    caches.open(CACHE_NAME).then(function (cache) {         // B7
      return cache.match(event.request).then(function (cachedResponse) {  // B8
        if (cachedResponse) return cachedResponse;          // B8 - return cache hit
        return fetch(event.request).then(function (networkResponse) {     // B8 - fetch if not cached
